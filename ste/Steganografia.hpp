...
void Encript::setText(char* text){
	this -> text = text;
}

void Encript::setOrignialImage(char* fileName){
	this -> first_fileName = fileName;
}

void Encript::setDestImage(char* fileName){
	this -> second_fileName = fileName;
}
...
int Encript::run(){
	...
	FILE* second_image;
		...
		printf("Encode...\n");
		first_image = fopen(this -> first_fileName,"rb"); /*apertura dei file*/
		if(first_image == NULL){
			printf("Error: Can't open the SourceImage\n");
			return -2;
		}
		second_image = fopen(this -> second_fileName,"wb+"); /*sia read and write*/
		if(second_image == NULL){
			printf("Error: Can't create/write the DestinationImage\n");
			return -3;
		}
		text = fopen(this -> text,"rb");
		if(text == NULL){
...
void Decript::setText(char* text){
	this -> text = text;
}

void Decript::setOrignialImage(char* fileName){
	this -> first_fileName = fileName;
}
...
int Decript::run(){
	...
	first_image = fopen(this -> first_fileName,"rb");
	if(first_image == NULL){
		printf("Error: Can't open the SourceImage\n");
		return -2;
	}
	text = fopen(this -> text,"w");
	if(text == NULL){
		printf("Error: Can't open the text\n");
		return -4;
	}
