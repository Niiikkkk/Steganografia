...
int* ByteHandler::getPoints(int textLength,int imgLength){
	if((imgLength/8)-54 < textLength){
		printf("Immagine troppo piccola e testo troppo lungo\n");
		exit(-1);
	}
	int* points = (int*)malloc(sizeof(int)*textLength*8+1);  /*+1 perchè la 0 è già occupata della length*/
	points[0] = textLength;
	for(int i = 1 ; i< textLength*8+1; i++){
		points[i] = i;
	}
	return points;
}