...
int main(int argc , char** argv){
	...
	if(modality){  /*Modalita di Encode , metto nell'immagine il messaggio*/
		...
		int* points = byte -> getPoints(textLength,firstLength);  /*si prende tutti i punti, su cui poi andare a mettere i byte modificati*/

		fseek(second_image,dataOffset,0); /*la fseek sposta il puntatore sul secondo paramentro , partendo dal terzo*/
		fputc(points[0],second_image); /*mette in una posizione fissa , dove iniziano i dati, la lunghezza del testo , che è in points[0]*/
		...
		int* points = byte ->getPoints(textLength,imageLength); /*si prende i punti*/
